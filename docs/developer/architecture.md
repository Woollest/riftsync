...
直接相性データが3件以上ある場合は、OP.GGのシナジーページから得た候補をそのまま表示順で使わず、候補単体のロール勝率に対するペア勝率の伸び、試合数補正、LoLalytics同ロール一致ボーナスを反映した補正後ペア強度を優先する。直接相性が足りない場合は、ロール別統計とチャンピオンのタグ/戦闘プロフィールから推定候補を補う。
